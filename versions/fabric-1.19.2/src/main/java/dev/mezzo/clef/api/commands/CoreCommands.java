    ...
    public static void registerAll(CommandDispatcher d) {
        ...
        d.register("chat", "send chat, or a /command if prefixed with '/' {message}", ctx -> {
            ...
            return ctx.onMain(() -> {
                ...
                // Outbound chat goes through the player here, not the connection: 1.19.2 signs
                // messages client-side, and ClientPacketListener.sendChat/sendCommand only exist
                // from 1.19.3 on. commandUnsigned() sends and reports true when the command has
                // no signable arguments; anything else has to take the signing path.
                if (msg.startsWith("/")) {
                    String cmd = msg.substring(1);
                    if (!mc.player.commandUnsigned(cmd)) mc.player.commandSigned(cmd, null);
                } else {
                    mc.player.chatSigned(msg, null);
                }
                JsonObject o = new JsonObject();
