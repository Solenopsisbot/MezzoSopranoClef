...
/**
 * NeoForge flavour: deliberately not shared with the Fabric target for this release, because
 * NeoForge patches Blaze3D with extension interfaces of its own ({@code GpuDeviceExtension}, plus
 * {@code RenderPass.setViewport} and {@code CommandEncoder.clearStencilTexture}) that are
 * NeoForge-only types. The GPU-free stack is per-loader, not per-release.
 *
 * <p>Headless, GPU-free implementation of Minecraft's {@link GpuDevice} for the <b>1.21.x</b> Blaze3D
...
public final class NoGlDevice implements GpuDevice {
    ...
    // --- NeoForge's GpuDeviceExtension. No GPU here, so advertise no optional features: enabling
    // one would send callers down a path this stub does not implement.
