...
/**
 * Headless, GPU-free {@link GpuDevice} for the <b>1.21.5–1.21.8</b> Blaze3D generation, NeoForge
 * flavour.
 *
 * <p>Deliberately not shared with the Fabric target for this release. NeoForge patches Blaze3D with
 * its own extension interfaces, so the device must satisfy more than vanilla's: {@code
 * GpuDeviceExtension} adds {@code deviceProperties()} and {@code enabledFeatures()}, {@code
 * RenderPass} gains {@code setViewport}, and {@code CommandEncoder} gains {@code
 * clearStencilTexture}. Those types are NeoForge-only, so one file cannot compile on both loaders —
 * this is genuinely per-loader, not per-release.
 *
...
public final class NoGlDevice implements GpuDevice {
    ...
    // --- NeoForge's GpuDeviceExtension. There is no GPU here, so report a backend that advertises
    // nothing: enabling any optional feature would invite callers down a path we do not implement.
    @Override public GpuDeviceFeatures enabledFeatures() { return () -> false; }

    @Override
    public GpuDeviceProperties deviceProperties() {
        return new GpuDeviceProperties() {
            @Override public String backendName() { return "MezzoSopranoClef no-gl"; }
            @Override public String apiName() { return "none"; }
        };
    }
    ...
    static final class NoGlRenderPass implements RenderPass {
        ...
        // NeoForge addition; nothing is rasterised, so the viewport is irrelevant.
    ...
    static final class NoGlCommandEncoder implements CommandEncoder {
        ...
        // NeoForge addition, alongside vanilla's colour/depth clears.
        @Override public void clearStencilTexture(GpuTexture t, int value) {}
