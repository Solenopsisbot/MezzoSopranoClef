...
import dev.mezzo.clef.mixin.client.KeyMappingAccessor;
...
public final class CombatController {
     ...
     */
    /** Hotbar slots are inventory indices 0-8 in every release this matrix builds. */
    ...
    public CompletableFuture<Result> startShoot(Minecraft mc, ShootRequest request) {
        ...
        if (!(player.getMainHandItem().getItem() instanceof BowItem)) {
            // This release's module has no Hotbar helper — the same reason `place` takes no
            // {item} here — so the bow has to be on the hotbar already. Say that plainly rather
            // than half-working: swapping one in from the bag needs slot-click plumbing this
            // module deliberately does not carry.
            int slot = -1;
            for (int i = 0; i < HOTBAR_SLOTS; i++) {
                if (player.inventory.getItem(i).getItem() instanceof BowItem) {
                    slot = i;
                    break;
                }
            }
            if (slot < 0) throw new ApiException(ErrorCode.MISSING_ITEM, "no bow on the hotbar");
    ...
    private static final class ShootJob extends Job {
        ...
        private boolean draw(Minecraft mc, LocalPlayer player, Entity target) {
            ...
            ((KeyMappingAccessor) mc.options.keyUse).clef$setDown(true);
            ...
            if (drawn <= 0 && phaseTicks > DRAW_START_SLACK_TICKS) {
                ...
                stopped = "blocked";
                detail = mc.screen != null
                        ? "the draw never started — a screen is open (" + mc.screen.getClass().getSimpleName() + ")"
            ...
            ((KeyMappingAccessor) mc.options.keyUse).clef$setDown(false);
        ...
        @Override
        void release(Minecraft mc) {
            if (mc.options != null) ((KeyMappingAccessor) mc.options.keyUse).clef$setDown(false);
    ...
    public static Entity findEntity(Minecraft mc, int id) {
        // Level.dragonParts() only exists from 1.21.4, so on this release a dragon *part* id
        // cannot be resolved back from the level. It does not need to be: `entities` reports the
        // dragon's own id, and meleeWhile picks the part to hit out of getSubEntities() itself.
    ...
    private static void point(Minecraft mc, LocalPlayer player, Vec3 point,
        ...
        // Entity.setYRot/setXRot arrive in 1.17.1; before that the rotation is a public field.
        player.yRot = yaw;
