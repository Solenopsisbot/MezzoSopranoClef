...
import dev.mezzo.clef.mixin.client.UserAccessor;
    ...
    public static void registerAll(CommandDispatcher d) {
        ...
        d.register("status", "bot, world and player status", ctx -> ctx.onMain(() -> {
            ...
            if (inWorld) {
                ...
                p.addProperty("x", mc.player.x);
                p.addProperty("y", mc.player.y);
                p.addProperty("z", mc.player.z);
                p.addProperty("yaw", mc.player.yRot);
                p.addProperty("pitch", mc.player.xRot);
                ...
                p.addProperty("selectedSlot", mc.player.inventory.selected);
                var held = mc.player.getMainHandItem();
                p.addProperty("heldItem", held.isEmpty() ? "empty"
                        : net.minecraft.core.Registry.ITEM.getKey(held.getItem()) + " x" + held.getCount());
                p.addProperty("dimension", net.minecraft.world.level.dimension.DimensionType.getName(mc.level.getDimension().getType()).toString());
                o.add("player", p);
            }
        ...
        d.register("auth.status", "current logged-in identity", ctx -> ctx.onMain(() -> {
            ...
            // No public getter for the account type on this release, so read the field.
            o.addProperty("type", ((UserAccessor) (Object) s).clef$getType().name());
            return o;
        }));
        ...
        d.register("look", "set view direction {yaw, pitch}", ctx -> {
            ...
            return ctx.onMain(() -> {
                ...
                if (!Float.isNaN(pitch)) mc.player.xRot = pitch;
                JsonObject o = new JsonObject();
                o.addProperty("yaw", mc.player.yRot);
                o.addProperty("pitch", mc.player.xRot);
                return o;
            });
        });
