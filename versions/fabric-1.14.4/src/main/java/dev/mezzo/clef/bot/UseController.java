 ...
 * release it (which fires a charged bow / stops blocking). {@link #tick(Minecraft)} runs
 * every client tick.
 */
public final class UseController {
    ...
    public void tick(Minecraft mc) {
        ...
        if (heldTicks > 0) {
            ...
            if (--heldTicks == 0) {
                ((KeyMappingAccessor) mc.options.keyUse).clef$setDown(false); // release => bow fires / use stops
            }
        } else if (releaseNow) {
            releaseNow = false;
