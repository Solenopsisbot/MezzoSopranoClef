...
import dev.mezzo.clef.mixin.client.AbstractWidgetAccessor;
    ...
    public static void registerAll(CommandDispatcher d) {
            ...
            if (sc != null) {
                ...
                for (GuiEventListener e : sc.children()) {
                    if (e instanceof AbstractWidget w) {
                        ...
                        jw.addProperty("text", w.getMessage());
        ...
        d.register("clickButton", "click a screen widget by its index (from `screen`) {index}", ctx -> {
            ...
            return ctx.onMain(() -> {
                ...
                w.mouseClicked(w.x + w.getWidth() / 2.0,
                        w.y + ((AbstractWidgetAccessor) w).clef$getHeight() / 2.0, 0);
                JsonObject o = new JsonObject();
                o.addProperty("clicked", index);
                o.addProperty("text", w.getMessage());
                return o;
            });
        });
        ...
        d.register("serverui", "read server-side UI: title/subtitle/action-bar, boss bars, scoreboard sidebar", ctx -> ctx.onMain(() -> {
            ...
            try {
                ...
                title.addProperty("title", hud.clef$getTitle());
                title.addProperty("subtitle", hud.clef$getSubtitle());
                title.addProperty("actionBar", hud.clef$getOverlayMessage());
                o.add("title", title);
