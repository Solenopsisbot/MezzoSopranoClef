...
public final class CoreCommands {
    ...
    public static void registerAll(CommandDispatcher d) {
        ...
        d.register("disconnect", "leave the current world", ctx -> ctx.onMain(() -> {
            // This era: disconnect(Screen returnTo) — the "transferring" flag arrived with transfers.
    ...
    private static JsonObject playerStatus(Minecraft mc) {
        ...
        p.addProperty("selectedSlot", player.getInventory().selected);
        ...
        for (MobEffectInstance effect : player.getActiveEffects()) {
            ...
            // getEffect() returns the effect itself here (a Holder only from 1.20.5), so the id
            // comes from the registry rather than off the holder.
            e.addProperty("id", net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT
    ...
    private static JsonObject worldStatus(Minecraft mc) {
        ...
        w.addProperty("dimension", world.dimension().location().toString());

        BlockPos feet = mc.player.blockPosition();
        world.getBiome(feet).unwrapKey().ifPresent(key -> w.addProperty("biome", key.location().toString()));
