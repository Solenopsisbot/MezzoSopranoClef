        ...
        switch (phase) {
            ...
            case WAIT_RESULT -> {
                ...
                if (!output.getItem().isEmpty()) {
                    mc.gameMode.handleInventoryMouseClick(syncId, output.index, 0, ClickType.QUICK_MOVE, mc.player);
                    phase = Phase.SETTLE;
                    phaseTicks = 0;
            ...
            case CLEANUP -> {
                ...
                if (!cleanupClicked) {
                    ...
                    for (Slot input : handler.getInputGridSlots()) {
                        ...
                        mc.gameMode.handleInventoryMouseClick(syncId, input.index, 0, ClickType.QUICK_MOVE, mc.player);
                    }
                    phaseTicks = 0;
                    return;
                }
