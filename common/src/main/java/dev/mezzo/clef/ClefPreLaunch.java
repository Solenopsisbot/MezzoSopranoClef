...
public final class ClefPreLaunch {
    ...
    public static void run() {
        ...
        // Older targets have no stub-able GPU device (Minecraft drove OpenGL directly, and the
        // Blaze3D device abstraction changed shape almost every release while it settled). Those
        // builds still run headless — the world/GUI draw is skipped — they just need a real GL
        // context, i.e. a hidden window (or Xvfb/Mesa on a bare server).
        boolean noGl = cfg.headless && cfg.noGl && !wantsGlBackend && VersionCapabilities.SUPPORTS_NO_GL;
        if (cfg.headless && cfg.noGl && !wantsGlBackend && !VersionCapabilities.SUPPORTS_NO_GL) {
            MezzoClef.LOG.info("preLaunch: GPU-free (no-gl) boot is not available on Minecraft {} — "
                    + "running headless with a hidden window instead.", VersionCapabilities.MINECRAFT);
        }
