...
import dev.mezzo.clef.util.Json;
...
public final class ControlServer implements WsServer.Listener {
    ...
    private static final Set<String> READ_ONLY_COMMANDS = Set.of(
            ...
            // Union of both sides: "protocol" is ours (ViaFabricPlus target reporting), the rest
            // of the read-only set came with main's agent API.
    ...
    public void onMessage(WsConnection conn, String message) {
        JsonObject req;
        try {
            req = Json.parse(message).getAsJsonObject();
        } catch (Exception e) {
